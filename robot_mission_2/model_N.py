...
from agents_N import GreenRobot, YellowRobot, RedRobot  
from objects_N import Waste , WasteDisposalZone, Radioactivity 
import random
import math
...
class RobotMission(Model):
    ...
    def choose_next_position_Snake(self, agent):
        """Choisit la prochaine position en suivant un quadrillage."""
        current_x, current_y = agent.pos
        if isinstance(agent, GreenRobot):
            grid_width = self.grid.width//3
            grid_height = self.grid.height   # Disposal zone for GreenRobot is at the end of z1
        elif isinstance(agent, YellowRobot):
            grid_width = 2*self.grid.width//3
            grid_height = self.grid.height# Disposal zone for YellowRobot is at the end of z2
        elif isinstance(agent, RedRobot):
            grid_width = self.grid.width
            grid_height = self.grid.height
        ...
        possible_steps = self.grid.get_neighborhood((current_x, current_y), moore=False, include_center=False)
        
        if current_x -1 == 0 and current_y == grid_height-1 : 
            next_position = (current_x -1, current_y)
        elif current_x == 0 and current_y != 0:
                next_position = (current_x, current_y - 1)
        # Vérifier le comportement de déplacement en fonction de la position actuelle
        elif current_y % 2 == 0:
        # Ligne paire : déplacer de gauche à droite
            if current_x < grid_width - 1 and (current_x + 1, current_y) in possible_steps:
                next_position = (current_x + 1, current_y)
            else:
            # Atteint le bord droit : repartir vers le bas
                next_position = (current_x, current_y + 1)
        elif current_y % 2 ==1:
        # Ligne impaire : déplacer de droite à gauche
            if current_x-1 == 0 and (current_x - 1, current_y) in possible_steps: 
                next_position = (current_x, current_y +1)
            elif current_x > 0 and (current_x - 1, current_y) in possible_steps:
                next_position = (current_x - 1, current_y)
            else:
            # Atteint le bord gauche : repartir vers le bas
                next_position = (grid_width - 1, current_y + 1)
        ...
        # # Calculer la prochaine position dans le quadrillage
        # if current_x < grid_width - 1 and (current_x + 1, current_y) in possible_steps and current_y % 2 == 0 :
        #     # Déplacer vers la droite quand la coordonnée est paire 
        #     next_position = (current_x + 1, current_y)
        # elif current_x < grid_width - 1 and (current_x - 1, current_y) in possible_steps and current_y % 2 == 1 :
        #     # Déplacer vers la gauche quand la coordonnée est paire 
        #     next_position = (current_x - 1, current_y)
        #     #if not self.is_position_allowed(agent, next_position) : 
        # elif current_y < grid_height - 1 and (current_x, current_y + 1) in possible_steps:
        #     # Si on atteint la fin de la ligne, passer à la ligne suivante
        #     next_position = (current_x, current_y + 1)
        # else:
        #     # Si on atteint la fin de la grille, rand
        ...
        return next_position
    #La fonction suivante permet d'aller à un point cible 
    def choose_next_position(self, agent, target_x, target_y):
        """Choisit la prochaine position pour se rapprocher de (target_x, target_y)."""
        current_x, current_y = agent.pos
        grid_width = self.grid.width
        grid_height = self.grid.height
        
        # Calculer la distance euclidienne actuelle par rapport à la cible
        current_distance = math.sqrt((current_x - target_x)**2 + (current_y - target_y)**2)
        
        # Initialiser les variables pour trouver la meilleure prochaine position
        best_next_position = agent.pos
        best_distance = current_distance
        
        # Parcourir toutes les positions voisines possibles
        possible_steps = self.grid.get_neighborhood(agent.pos, moore=False, include_center=False)
        for next_position in possible_steps:
            next_x, next_y = next_position
            
            # Calculer la distance euclidienne de la prochaine position à la cible
            next_distance = math.sqrt((next_x - target_x)**2 + (next_y - target_y)**2)
            
            # Si cette prochaine position réduit la distance à la cible, la choisir comme la meilleure
            if next_distance < best_distance:
                best_next_position = next_position
                best_distance = next_distance
        
        # Retourner la meilleure prochaine position trouvée
